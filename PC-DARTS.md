...
在此图中解释了信息如何传输到 node3,在模型搜索的过程中主要有2组超参数：α 和 β，对于参数 α 我们只采样了输入的 1/K,剩余的直接 concat 到了下一个 stage.为了消除 1/k 采样造成的不确定影响我们加入了 β，最终是通过 α*β 来评估。
...
# 代码解释 #
有一些博客对代码进行了比较详细的分析这里选了几篇我在阅读代码中参考的博客：
-https://blog.csdn.net/yiran103/article/details/100052763
首先先吐槽下 pcdatrs 的函数命名与函数定义有些比较随意，导致读者一开始看有点混乱。代码其实分为2大类，一类为模型搜索一类是根据搜索出来的模型进行重新的训练，而搜索过程与模型重新训练过程中的网络定义也是不一样的。这里先梳理下2类流程

## 搜索过程
这里以 cifar10 数据集搜索为例子，这里需要明确3个参数的含义，第一个是网络的权重 w,就是我们平时进行模型训练的权重，第二个是搜索网络结构引入的2个超参数 α 和 β，这里对应代码中的 weight 与 weight2 (吐槽的点：命名真的很随意啊)，这里还涉及到论文中说的联合优化的问题，及时当需要更新 W 的参数的时候，需要固定 α 和 β，当需要更新 α 和 β 的时候就需要固定 W 参数，因此在搜索的代码中可以看到会分2步。

- 网络定义,网络的定义在 model_search.py 中这里会定义 Cell 与 Network
- 进行数据处理经训练的数据分成 2份 一份是训练集一份是验证集
- 进行网络的训练（w 权重的更新与 α 和 β 的搜索 ）
- α 和 β 的更新在 architect.py 函数中
- w 的更新就是使用 SGD 优化
- 
